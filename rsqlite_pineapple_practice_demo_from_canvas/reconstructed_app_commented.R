# PineApple BI Dashboard (commented version)
#
# This file keeps the same app logic as reconstructed_app.R and adds explanatory
# comments so each section is easier to follow.

# Load the packages used by the app:
# - DBI / RSQLite: connect to and query the SQLite database
# - shiny / shinythemes / shinyalert: app framework, theme, and popup alerts
# - DT: interactive data tables
# - ggplot2 / plotly: static and interactive charts
# - forecast: provides geom_forecast() used in the prediction plot
# - dplyr: available for data wrangling if needed
# - scales: currency formatting helpers such as dollar()
library(DBI)
library(RSQLite)
library(shiny)
library(shinythemes)
...
# Fail early if the SQLite database is not in the same working folder as the app.
...
# Connect to the database once at app startup.
...
# Stop with a clear message if the database does not contain any product types.
if (nrow(prodtype) == 0) {
...
# Helper: format a number with commas and a configurable rounding level.
...
# Helper: format a value as currency.
...
# Helper: create a KPI card with a title and a uiOutput placeholder.
...
ui <- tagList(
  ...
  navbarPage(
    ...
    # About tab: simple descriptive text explaining what the app contains.
    tabPanel(
      title = "About",
      fluidPage(
        br(),
    ...
    # Overview tab: top-line KPIs, daily sales chart, and filtered transaction table.
    tabPanel(
      title = "Overview",
      fluidPage(
        br(),
    ...
    # Store Info tab: map + top 10 stores table + button to persist the top 10 result.
    tabPanel(
      title = "Store Info",
      fluidPage(
        br(),
    ...
    # Sales Prediction tab: forecast the next 7 days for the selected product type.
    tabPanel(
