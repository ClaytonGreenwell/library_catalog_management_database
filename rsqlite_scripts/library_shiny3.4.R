...
ui <- tagList(
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          selectInput("return_filter_patron", "Filter by Patron:", choices = NULL),
...
server <- function(input, output, session) {
  ...
  output$books_return_table <- renderDT({
    df <- checked_out_books() %>%
      select(Book_id, Title, Author_Names, Bookshelves, Subjects, Patron_Name, Due_Date)
