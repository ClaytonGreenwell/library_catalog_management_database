...
ui <- tagList(
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          ui_space("10px"),
          actionButton("checkout_btn", "Checkout", class = "btn-primary", width = "100%")
        ),
    ...
    tabPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          selectInput("return_filter_patron", "Select Patron:", choices = NULL),
          ui_space("10px"),
          actionButton("return_btn2", "Mark Book as Returned", class = "btn-warning", width = "100%")
        ),
...
server <- function(input, output, session) {
  ...
  load_data <- function() {
    ...
    rv$transactions <- fix_dates(
      dbGetQuery(
        db,
        "SELECT t.Transaction_id, t.Patron_id, p.First_Name, p.Last_Name,
                t.book_id AS Book_id, b.Title, t.Checkout_Date, t.Due_Date, t.Return_Date
  ...
  observeEvent(input$checkout_btn, {
    ...
    dbExecute(
      db,
      "UPDATE books SET Available = 'No', Checked_Out_By = ?
       WHERE Book_id = ?",
  ...
  observeEvent(input$return_btn2, {
    ...
    dbExecute(
      db,
      "UPDATE transaction_log SET Return_Date = ?
    ...
    dbExecute(
      db,
      "UPDATE books SET Available = 'Yes', Checked_Out_By = NULL
       WHERE Book_id = ?",
      params = list(book_id)
    )
  ...
  observeEvent(input$add_book, {
    showModal(modalDialog(
      ...
      textInput("new_book_author", "Author Names"),
      textInput("new_book_shelf", "Bookshelves"),
      textInput("new_book_subject", "Subjects"),
      footer = tagList(
        modalButton("Cancel"),
  ...
  observeEvent(input$save_new_book, {
    dbExecute(
      db,
      "INSERT INTO books (Title, Author_Names, Bookshelves, Subjects, Available, Checked_Out_By)
       VALUES (?, ?, ?, ?, 'Yes', NULL)",
      params = list(
        input$new_book_title,
        input$new_book_author,
        input$new_book_shelf,
        input$new_book_subject
      )
    )
    removeModal()
    load_data()
  ...
  observeEvent(input$edit_book, {
    ...
    if (!length(sel)) {
      shinyalert("No Book Selected", "Pick a book to edit.", type = "warning")
      return()
    }
    ...
    showModal(modalDialog(
      ...
      textInput("edit_book_author", "Author Names", value = book_row$Author_Names),
      textInput("edit_book_shelf", "Bookshelves", value = book_row$Bookshelves),
      textInput("edit_book_subject", "Subjects", value = book_row$Subjects),
      footer = tagList(
        modalButton("Cancel"),
    ...
    observeEvent(input$save_edit_book, {
      dbExecute(
        db,
        "UPDATE books
         SET Title = ?, Author_Names = ?, Bookshelves = ?, Subjects = ?
         WHERE Book_id = ?",
        params = list(
          input$edit_book_title,
          input$edit_book_author,
          input$edit_book_shelf,
          input$edit_book_subject,
          book_row$Book_id
        )
      )
      removeModal()
  ...
  observeEvent(input$delete_book, {
    sel <- input$books_admin_rows_selected
    if (!length(sel)) {
      shinyalert("No Book Selected", "Pick a book to delete.", type = "warning")
      return()
    }
    ...
    if (count_trans > 0) {
      shinyalert("Cannot Delete Book", "This book has transaction history.", type = "error")
      return()
    }
    ...
    load_data()
    shinyalert("Book Deleted", "Record removed.", type = "success")
  })
