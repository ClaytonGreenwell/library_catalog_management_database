...
db <- dbConnect(SQLite(), "library.db")
...
server <- function(input, output, session) {
  ...
  books_with_status <- reactive({
    ...
    rv$books %>%
      mutate(
        ...
        # Hybrid check: both books.Available and transaction_log
  ...
  observeEvent(input$checkout_btn, {
    ...
    # Selected book from the displayed available books
    ...
    # Double-check via hybrid logic: books + transaction_log
    ...
    # Check transaction_log for active checkout
    ...
    # Parse selected patron "Last_Name, First_Name"
    ...
    patron_row <- rv$patrons %>%
      filter(Last_Name == last_name, First_Name == first_name)
    ...
    # Update books: Available = "No", Checked_Out_By = patron_id
    dbExecute(
      db,
      "UPDATE books
  ...
  observeEvent(input$return_btn, {
    ...
    # Update books: Available = "Yes", Checked_Out_By = NULL
    dbExecute(
      db,
      "UPDATE books
  ...
  # Delete Patron (blocked if any transaction history)
  ...
  output$books_admin <- renderDT({
    req(rv$books)
    datatable(rv$books, selection = "single", options = list(pageLength = 10))
  })
  
  # Add Book (minimal fields, defaults: Available = "Yes", Checked_Out_By = NULL)
  ...
  # Edit Book (descriptive fields only)
  ...
  # Delete Book (blocked if referenced in any transaction or currently checked out)
  observeEvent(input$delete_book, {
    ...
    # Block if referenced in any transaction (historical)
    count_trans <- dbGetQuery(
      db,
